...
/****** Object:  Table [dbo].[CommandLog]    Script Date: 10/30/2014 09:42:37 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[CommandExecute]    Script Date: 10/30/2014 09:42:48 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
  ...
  IF @LogToTable = 'Y'
  BEGIN
    INSERT INTO dbo.CommandLog (DatabaseName, SchemaName, ObjectName, ObjectType, IndexName, IndexType, StatisticsName, PartitionNumber, ExtendedInfo, CommandType, Command, StartTime, Comment)
    VALUES (@DatabaseName, @SchemaName, @ObjectName, @ObjectType, @IndexName, @IndexType, @StatisticsName, @PartitionNumber, @ExtendedInfo, @CommandType, @Command, @StartTime, @Comment)
  END
...
/****** Object:  StoredProcedure [dbo].[DatabaseBackup]    Script Date: 10/30/2014 09:42:49 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
  ...
  BEGIN
    SELECT @NumberOfFiles = CASE WHEN @URL IS NOT NULL THEN 1 ELSE (SELECT COUNT(*) FROM @Directories) END
  END
  ...
  IF @NumberOfFiles < 1 OR @NumberOfFiles > 64 OR (@NumberOfFiles > 32 AND @BackupSoftware = 'SQLBACKUP') OR @NumberOfFiles IS NULL OR @NumberOfFiles < (SELECT COUNT(*) FROM @Directories) OR @NumberOfFiles % (SELECT NULLIF(COUNT(*),0) FROM @Directories) > 0 OR (@URL IS NOT NULL AND @NumberOfFiles <> 1)
  BEGIN
  ...
  IF (@URL IS NOT NULL AND @Directory IS NOT NULL) OR (@URL IS NOT NULL AND @Version < 11.03339) OR (@URL IS NOT NULL AND @BackupSoftware IS NOT NULL) OR (@URL NOT LIKE 'https://%/%')
  BEGIN
  ...
  BEGIN
    ...
    BEGIN
      ...
      IF EXISTS (SELECT * FROM @CurrentDirectories)
      BEGIN
      ...
      BEGIN
        ...
        BEGIN
          ...
          IF @ReadWriteFileGroups = 'Y' AND @CurrentDatabaseName <> 'master' SET @CurrentCommand02 = @CurrentCommand02 + ' READ_WRITE_FILEGROUPS'

          SET @CurrentCommand02 = @CurrentCommand02 + ' TO'

          SELECT @CurrentCommand02 = @CurrentCommand02 + ' ' + [Type] + ' = N''' + REPLACE(FilePath,'''','''''') + '''' + CASE WHEN ROW_NUMBER() OVER (ORDER BY FilePath ASC) <> @NumberOfFiles THEN ',' ELSE '' END
          FROM @CurrentFiles
          ...
          IF @NoRecovery = 'Y' AND @CurrentBackupType = 'LOG' SET @CurrentCommand02 = @CurrentCommand02 + ', NORECOVERY'
          ...
          IF @URL IS NOT NULL SET @CurrentCommand02 = @CurrentCommand02 + ', CREDENTIAL = N''' + REPLACE(@Credential,'''','''''') + ''''
        END
        ...
        BEGIN
          ...
          SELECT @CurrentCommand02 = @CurrentCommand02 + ', @filename = N''' + REPLACE(FilePath,'''','''''') + ''''
          FROM @CurrentFiles
          ORDER BY FilePath ASC
          ...
          IF @ReadWriteFileGroups = 'Y' AND @CurrentDatabaseName <> 'master' SET @CurrentCommand02 = @CurrentCommand02 + ', @read_write_filegroups = 1'
        ...
        BEGIN
          ...
          SELECT @CurrentCommand02 = @CurrentCommand02 + ' DISK = N''' + REPLACE(FilePath,'''','''''') + '''' + CASE WHEN ROW_NUMBER() OVER (ORDER BY FilePath ASC) <> @NumberOfFiles THEN ',' ELSE '' END
          FROM @CurrentFiles
          ORDER BY FilePath ASC
        ...
        BEGIN
          ...
          SELECT @CurrentCommand02 = @CurrentCommand02 + ', ' + CASE WHEN ROW_NUMBER() OVER (ORDER BY FilePath ASC) = 1 THEN '@filename' ELSE '@backupfile' END + ' = N''' + REPLACE(FilePath,'''','''''') + ''''
          FROM @CurrentFiles
          ORDER BY FilePath ASC
          ...
          IF @ReadWriteFileGroups = 'Y' AND @CurrentDatabaseName <> 'master' SET @CurrentCommand02 = @CurrentCommand02 + ', @readwritefilegroups = 1'
      ...
      BEGIN
        ...
        BEGIN
          ...
          SELECT @CurrentCommand03 = @CurrentCommand03 + ' @filename = N''' + REPLACE(FilePath,'''','''''') + '''' + CASE WHEN ROW_NUMBER() OVER (ORDER BY FilePath ASC) <> @NumberOfFiles THEN ',' ELSE '' END
          FROM @CurrentFiles
          ORDER BY FilePath ASC
...
/****** Object:  StoredProcedure [dbo].[DatabaseIntegrityCheck]    Script Date: 10/30/2014 09:42:49 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[IndexOptimize]    Script Date: 10/30/2014 09:42:49 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[sp_WhoIsActive]    Script Date: 10/30/2014 09:42:49 ******/
SET ANSI_NULLS ON
GO
